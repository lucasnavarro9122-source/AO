"""Run opt-in QA for AOLighting2DV283 (Arte): Original / Mejorada / night screenshots in an open Unity editor, preserving local saves."""
...
backup = root / "Temp" / f"lighting-qa-saves-{time.time_ns()}"
...
report = root / "MigrationReports/lighting_v287.json"
if report.exists():
    report.replace(backup / "previous-result.json")
flag = root / "Temp/run_lighting_qa"
(root / "Temp/stop_lighting_qa").unlink(missing_ok=True)
...
try:
    while not report.exists():
        if time.monotonic() > deadline:
            raise TimeoutError("Unity lighting QA did not finish")
        time.sleep(1)
    time.sleep(1)
    result = json.loads(report.read_text(encoding="utf-8"))
    print(json.dumps(result, ensure_ascii=True, indent=1), flush=True)
    # The rendered view must match "Original" in layout: not moved, not duplicated (Arte, 25/09).
    from qa_view_check import compare, verdict
    from PIL import Image
    shots = root / "MigrationReports/lighting_v287"
    reference = shots / "luces_original.png"
    ...
    if box[2] <= 0 or box[0] + box[2] > width + 1 or box[1] + box[3] > height + 1:
        view_problems.append(f"viewport {box} fuera de la captura {width}x{height}")
    else:
        for name in ("luces_mejorada", "luces_noche_mejorada"):
            metrics = compare(reference, shots / f"{name}.png", box)
            problems = verdict(metrics)
            print(f"{name}: {metrics} {problems or 'OK'}", flush=True)
            view_problems += [f"{name}: {p}" for p in problems]
    if not result["passed"] or view_problems:
        raise AssertionError(f"Unity lighting QA failed: {len(result['failures'])} failures, {len(result['errors'])} log errors, "
