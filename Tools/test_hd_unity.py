"""Run opt-in QA for the HD map textures: Original vs HD with both lighting modes in an open Unity editor, preserving local saves."""
...
backup = root / "Temp" / f"hd-qa-saves-{time.time_ns()}"
...
report = root / "MigrationReports/hd_v288.json"
if report.exists():
    report.replace(backup / "previous-result.json")
flag = root / "Temp/run_hd_qa"
(root / "Temp/stop_hd_qa").unlink(missing_ok=True)
...
try:
    while not report.exists():
        if time.monotonic() > deadline:
            raise TimeoutError("Unity HD QA did not finish")
        time.sleep(1)
    time.sleep(1)
    result = json.loads(report.read_text(encoding="utf-8"))
    print(json.dumps(result, ensure_ascii=True, indent=1), flush=True)
    # HD must keep the layout of Original (same place, not moved, not duplicated) with both lighting modes.
    from qa_view_check import compare, verdict
    from PIL import Image
    shots = root / "MigrationReports/hd_v288"
    box = tuple(round(result.get(k, 0)) for k in ("vx", "vy", "vw", "vh"))
    view_problems = []
    for light in ("luz_original", "luz_mejorada"):
        reference = shots / f"hd_off_{light}.png"
        width, height = Image.open(reference).size
        if box[2] <= 0 or box[0] + box[2] > width + 1 or box[1] + box[3] > height + 1:
            view_problems.append(f"viewport {box} fuera de la captura {width}x{height}"); break
        metrics = compare(reference, shots / f"hd_on_{light}.png", box)
        problems = verdict(metrics)
        print(f"hd_on_{light} vs hd_off_{light}: {metrics} {problems or 'OK'}", flush=True)
        view_problems += [f"hd_on_{light}: {p}" for p in problems]
    if not result["passed"] or view_problems:
        raise AssertionError(f"Unity HD QA failed: {len(result['failures'])} failures, {len(result['errors'])} log errors, "
                             f"view: {view_problems}")
finally:
    if flag.exists():
        flag.unlink()
