"""Run opt-in controls QA in an open Unity editor, preserving local saves."""
...
backup = root / "Temp" / f"controls-qa-saves-{time.time_ns()}"
...
report = root / "MigrationReports/controls_v260.json"
if report.exists():
    report.replace(backup / "previous-result.json")
flag = root / "Temp/run_controls_qa"
...
try:
    while not report.exists():
        if time.monotonic() > deadline:
            raise TimeoutError("Unity controls QA did not finish")
        time.sleep(1)
    result = json.loads(report.read_text(encoding="utf-8"))
    print(json.dumps(result, ensure_ascii=True), flush=True)
    if not result["passed"]:
        raise AssertionError("Unity controls QA failed")
finally:
    if flag.exists():
        flag.unlink()
